...
## Corpus Check
- 103 files · ~47,798 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 876 nodes · 1575 edges · 57 communities (49 shown, 8 thin omitted)
...
1. `ActorContext` - 76 edges
2. `PatientId` - 30 edges
3. `authorize()` - 29 edges
4. `UserId` - 27 edges
5. `ClinicalService` - 27 edges
6. `BranchId` - 25 edges
7. `PatientRepository` - 25 edges
8. `actorFromRequest()` - 24 edges
...
### Community 4 - "patients/service.ts"
Cohesion: 0.06
Nodes (37): UserId, ConsultationSummary, SafeSummary, Consent, ConsentId, ConsentInput, EmergencyContact, Folio (+29 more)

### Community 5 - "clinical/service.ts"
Cohesion: 0.08
Nodes (34): AbandonConsultationInput, FullConsultation, LensUsage, NonClinicalNote, NonClinicalNoteKey, ReleaseConsultationInput, SetNonClinicalNoteInput, assertNonClinicalNoteKey() (+26 more)
...
Cohesion: 0.11
Nodes (19): magicast, devDependencies, magicast, prisma, @types/node, @types/react, @types/react-dom, typescript (+11 more)
...
### Community 29 - "clinical.contract.ts"
Cohesion: 0.07
Nodes (29): Consultation, ConsultationId, ConsultationStatus, Eye, IssuePrescriptionInput, OpenConsultationInput, Prescription, PrescriptionAmendmentInput (+21 more)
...
Cohesion: 0.14
Nodes (25): PATCH(), RouteContext, PATCH(), POST(), RouteContext, PATCH(), POST(), RouteContext (+17 more)

### Community 38 - "BranchId"
Cohesion: 0.09
Nodes (27): PrescriptionId, InventoryAdjustmentInput, InventoryMovement, MovementId, MovementReason, Product, ProductCategory, ProductCode (+19 more)
...
## Knowledge Gaps
- **403 isolated node(s):** `MovementId`, `ProductCategory`, `MovementReason`, `InventoryAdjustmentInput`, `ReconciliationItem` (+398 more)
...
- **Why does `ActorContext` connect `ActorContext` to `actorFromRequest`, `auth.contract.ts`, `patients/service.ts`, `clinical/service.ts`, `BranchId`, `clinical.contract.ts`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `UserId` connect `patients/service.ts` to `auth.contract.ts`, `clinical.contract.ts`, `BranchId`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `PatientRepository` connect `patients/service.ts` to `actorFromRequest`, `clinical/service.ts`, `clinical.contract.ts`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **What connects `MovementId`, `ProductCategory`, `MovementReason` to the rest of the system?**
